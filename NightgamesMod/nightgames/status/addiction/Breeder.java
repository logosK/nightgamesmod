...
public class Breeder extends Addiction {
    ...
    protected String describeDecrease() {
        switch (getSeverity()) {
            case LOW:
                return "You still feel a desire to fuck " + cause.getName() + " silly, but it's no longer a driving force"
                                + " in your mind.";
            case MED:
    ...
    protected String describeCombatDecrease() {
        return "You relish at letting loose inside of " + cause.getName() + ", subduing the instinctive needs for now.";
    }
    ...
    public String describeMorning() {
        return "Of course morning wood is pretty normal, but this morning it just won't go down! No"
                        ...
                        + " can do. " + cause.getName() + " is clearly the cause, so maybe " + cause.pronoun() + " can help?";
    }
